...
// visualize high diemnsional data: https://www.youtube.com/watch?v=ypc7Ul9LkxA
// move axis left and right, filter range per axis
// different range for each axis
// sort axis for specific point so that line desents across axis
// how to plot:
// instance buffer contains points, each instance has n points which correspond to n dimesnions
// index buffer is used for ordering of axis
// uniform buffer contains an array of point offsets
// to draw we get vertex index then index into uniform array to get offsets then match on index
// if index == 0 then get first point in instance
// if index == 1 then get second point in instance
// this will require shader specialization because we need to know how many axis we will plot
// each instance could also have a color
// a maximum of 16 vertex attributes are supported
// max vertex buffer array stride is 2k
...
use epaint::text::{FontDefinitions, Fonts, Row};
use epaint::{
    ClippedMesh, Color32, Galley, Mesh, Shape, Stroke, TessellationOptions, Tessellator, TextStyle,
};
...
//        });
//        //        render_pass0.set_viewport(0., 0., 400., 400., 0., 1.);
//        let tick_size = 5;
